...
def create_url(zipcode, filter):
    ...
    #elif url == url:
     #   url = 'https://www.zillow.com/san-diego-ca-92115/?fromHomePage=true&shouldFireSellPageImplicitClaimGA=false&fromHomePageTab=buy&searchQueryState=%7B%22pagination%22%3A%7B%7D%2C%22usersSearchTerm%22%3A%2292115%22%2C%22mapBounds%22%3A%7B%22west%22%3A-117.12299070410157%2C%22east%22%3A-117.01724729589844%2C%22south%22%3A32.72025975057221%2C%22north%22%3A32.790998489812324%7D%2C%22mapZoom%22%3A13%2C%22regionSelection%22%3A%5B%7B%22regionId%22%3A96657%2C%22regionType%22%3A7%7D%5D%2C%22isMapVisible%22%3Afalse%2C%22filterState%22%3A%7B%22ah%22%3A%7B%22value%22%3Atrue%7D%2C%22sort%22%3A%7B%22value%22%3A%22globalrelevanceex%22%7D%7D%2C%22isListVisible%22%3Atrue%7D'
    else:
...
def write_data_to_csv(properties_list):
    # saving scraped data to csv.

    with open("properties-%s.csv" % (zipcode), 'wb') as csvfile:
        fieldnames = ['title', 'address', 'city', 'state', 'postal_code', 'price', 'facts and features',  'url',
                      'agentName','agentEmail','agentPhoneNumber','brokerName','brokerPhoneNumber']#'real estate provider',
...
def write_data_to_excel(properties_list):
...
def get_data_from_json(raw_json_data):
    ...
    try:
        ...
        for properties in search_results:
            ...
            #print(property_url)
            if property_url:
                response = requests.get(property_url, headers=get_headers())
                print("status code received:", response.status_code)
                ...
                search_zpid = json_data.get('zpid')
                #print(search_zpid)
                ...
                mylist = list(results)
                #mylist = list(results.items())
                #print("mylist:", type(mylist))
                list1 = ((mylist)[1])
                #print(type(list1))
                #print(list1)
                pfone_number = results.get(list1)
                #print(type(pfone_number))
                #print(pfone_number.keys())
                #property = results.get(list1.get('property')
                #print(type(property))
                #print("property:", property)
             
                data1 = pfone_number.get('property').get('attributionInfo')
                #print(data.keys())
                '''
                agentName = data1.get('agentName')
                agentEmail = data1.get('agentEmail')
                agentPhoneNumber = data1.get('agentPhoneNumber')
                brokerName = data1.get('brokerName')
                brokerPhoneNumber = data1.get('brokerPhoneNumber')
                '''
                Agent_info = {
                'agentName': data1.get('agentName'),
                'agentEmail': data1.get('agentEmail'),
                ...
                #properties_list.append(Agent_info)
                #print(Agent_info)
                #print(type(data))
