...
response = BeautifulSoup(requests.get(url, headers=headers).content, "html5lib")
#data = response.find("script", {"data-zrr-shared-data-key" : "mobileSearchPageStore"})
#for d in data:
#pprint(data)
'''
response = requests.get(url, headers=headers)
#print("status code received:", data.status_code)
...
#search_results = parser.xpath('//div[@class="list-card-info"]')
search_results = response.find('//div[@class="list-card-info"]')
...
if not search_results:
        ...
        #raw_json_data = parser.xpath('//script[@data-zrr-shared-data-key="mobileSearchPageStore"]//text()')[3]
        #raw_json_data = parser.xpath('//script[@type="application/json"]//text()')#[2]
        ...
        #print(json_data.get("cat1").get('searchResults').get('listResults', []))
